...
impl SurrealDatabase {
    pub(crate) async fn create(
        ...
        // Only run installation if INSTALL env var is set to true
        if let Ok(install) = env::var("INSTALL") {
            if install.to_lowercase() == "true" {
                //Setup db for incoming messages
                let install_query = include_str!("surql/install.surql");
                DB.query(install_query).await?;
            }
        }
        Ok(instance)
    }
        ...
        let handle = std::thread::spawn(move || {
            ...
            rt.block_on(async move {
                ...
                let mut results = DB.query(
                    include_str!("surql/create_persons_threads.surql")
                ).await?;

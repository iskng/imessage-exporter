...
use imessage_database::{
    ...
    tables::{
        ...
        table::{FITNESS_RECEIVER, ME, Table, YOU},
    },
    util::{
        dates::{TIMESTAMP_FACTOR, format, get_local_time, get_utc_time, readable_diff},
...
impl<'a> DB<'a> {
    fn write_message(&mut self, message: &Message) -> Result<(), RuntimeError> {
        ...
        let full_message = if message.is_announcement() {
            self.format_announcement(message)
        } else {
            self.format_message(message, 0)
                .map_err(RuntimeError::DatabaseError)?
        };
                ...
                let res = if message.is_url() {
                    ...
                    match AppMessage::from_map(&parsed) {
                        Ok(bubble) => match balloon {
                            ...
                            CustomBalloon::Polls => unreachable!(),
...
impl<'a> DB<'a> {
    ...
    fn format_poll(&self, poll: &Poll, indent: &str) -> String {
        let mut out_s = String::from(indent);

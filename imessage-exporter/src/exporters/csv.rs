...
const _DELIMITER: &str = "\t";
const HEADERS: &str =
    "rowid\tguid\ttext\tservice\thandle_id\tdestination_caller_id\tsubject\tdate\tdate_read\t\
                      date_delivered\tis_from_me\tis_read\titem_type\tother_handle\tshare_status\tshare_direction\t\
                      group_title\tgroup_action_type\tassociated_message_guid\tassociated_message_type\t\
                      balloon_bundle_id\texpressive_send_style_id\tthread_originator_guid\tthread_originator_part\t\
                      date_edited\tassociated_message_emoji\tchat_id\tnum_attachments\tdeleted_from\tnum_replies\t\
                      formatted_text\tattachment_paths";

impl<'a> Exporter<'a> for CSV<'a> {
    fn new(config: &'a Config) -> Result<Self, RuntimeError> {
        let mut orphaned = config.options.export_path.clone();
        orphaned.push(ORPHANED);
        orphaned.set_extension("csv");
        ...
        // Write headers to orphaned file
        writeln!(csv.orphaned, "{}", HEADERS).map_err(RuntimeError::DiskError)?;
        ...
        match self.config.conversation(message) {
            Some((chatroom, _)) => {
                ...
                if !self.files.contains_key(&filename) {
                    ...
                    writeln!(writer, "{}", HEADERS).map_err(RuntimeError::DiskError)?;
                    self.files.insert(filename.clone(), writer);
                }
                Ok(self.files.get_mut(&filename).unwrap())
            }
...
impl<'a> CSV<'a> {
    fn write_message(&mut self, message: &Message) -> Result<(), RuntimeError> {
        ...
        let attachment_paths = {
            ...
            escape_field(&paths.join(";"))
        };

        // Now get the writer and write everything
        ...
        writeln!(
            ...
            "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}",
            message.rowid,
            escape_field(&message.guid),
            escape_field(&message.text.as_deref().unwrap_or("")),
            escape_field(&message.service.as_deref().unwrap_or("")),
            message.handle_id.unwrap_or(-1),
            escape_field(&message.destination_caller_id.as_deref().unwrap_or("")),
            escape_field(&message.subject.as_deref().unwrap_or("")),
            date,
            date_read,
            date_delivered,
            message.is_from_me,
            ...
            escape_field(&message.group_title.as_deref().unwrap_or("")),
            message.group_action_type,
            escape_field(&message.associated_message_guid.as_deref().unwrap_or("")),
            message.associated_message_type.unwrap_or(-1),
            escape_field(&message.balloon_bundle_id.as_deref().unwrap_or("")),
            escape_field(&message.expressive_send_style_id.as_deref().unwrap_or("")),
            escape_field(&message.thread_originator_guid.as_deref().unwrap_or("")),
            escape_field(&message.thread_originator_part.as_deref().unwrap_or("")),
            date_edited,
            escape_field(&message.associated_message_emoji.as_deref().unwrap_or("")),
            message.chat_id.unwrap_or(-1),
            message.num_attachments,
            message.deleted_from.unwrap_or(-1),
            message.num_replies,
            formatted_text,
            attachment_paths
...
/// Escapes a field for tab-separated output by replacing tabs with spaces
/// and wrapping in quotes if it contains newlines
fn escape_field(field: &str) -> String {
    let field = field.replace('\t', "    "); // Replace tabs with 4 spaces
    if field.contains('\n') || field.contains('"') {
...
impl<'a> BalloonFormatter<&'a str> for CSV<'a> {
